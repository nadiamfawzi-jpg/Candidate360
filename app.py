...
with delivery_tab:
    ...
    st.write(
        "Prepare the camera and select START. You can return to Interview "
        "Room while the camera remains active, record your answer, then come "
        "back here, select STOP and create the live-camera evaluation."
    )

    if "live_expression_analyser" not in st.session_state:
        if st.button(
            "Prepare live facial-expression camera",
            use_container_width=True
        ):
            try:
                with st.spinner("Loading the facial-expression model..."):
                    from vision_utils import LiveExpressionAnalyser
                    st.session_state.live_expression_analyser = (
                        LiveExpressionAnalyser(get_expression_model())
                    )
            except Exception as error:
                st.error(str(error))

    if "live_expression_analyser" in st.session_state:
        try:
            from streamlit_webrtc import webrtc_streamer

            live_analyser = st.session_state.live_expression_analyser
            webrtc_streamer(
                key="candidate360_live_expression_camera",
                video_frame_callback=live_analyser.process_frame,
            ...
            with live_first:
                if st.button(
                    "Create live-camera evaluation",
                    use_container_width=True
                ):
                    live_result = live_analyser.get_results()
                    if live_result.get("analysed_frames", 0) == 0:
                        st.warning(
                            "Start the camera and keep it running for a few "
                            "seconds first."
                        )
                    else:
                        st.session_state.delivery_result = live_result
                        st.success("Live-camera evaluation created.")
            with live_second:
                if st.button(
                    "Reset live-camera observations",
                    use_container_width=True
                ):
                    live_analyser.reset()
                    if st.session_state.get("delivery_result", {}).get(
                        "source"
                    ) == "live_camera":
            ...
            st.error(
                "Live camera is unavailable. Add streamlit-webrtc to "
