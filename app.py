...
from candidate360_api import build_interview, create_session_summary, evaluate_answer
...
TRANSCRIPTION_LANGUAGES = [
    ...
    "English", "Arabic", "Afrikaans", "Albanian", "Amharic",
    "Armenian", "Assamese", "Azerbaijani", "Bashkir", "Basque",
    "Belarusian", "Bengali", "Bosnian", "Breton", "Bulgarian",
    "Burmese", "Cantonese", "Catalan", "Chinese", "Croatian",
...
def release_speech_model():
    """Release Whisper before loading the visual-model group."""
    clear_method = getattr(get_speech_model, "clear", None)
    if callable(clear_method):
        clear_method()
    gc.collect()


def analyse_complete_uploaded_video(
    uploaded_video,
    language,
    speech_api_key=""
):
    """Run visual and speech stages independently on one uploaded video."""
    from speech_utils import (
        evaluate_voice_delivery_path,
        extract_audio_from_video,
        transcribe_audio_path
    )
    from vision_utils import analyse_video, save_upload
    ...
    try:
        try:
            release_speech_model()
            pose_model = get_pose_model()
            expression_model_data = get_expression_model()
            confidence_model_data = get_confidence_model()
            try:
                gesture_recognizer = get_gesture_recognizer()
            except Exception as error:
                gesture_recognizer = None
                outcome["gesture_warning"] = (
                    "Hand gestures were skipped: " + str(error)
                )

            outcome["visual_result"] = analyse_video(
                video_path,
                pose_model,
            ...
            outcome["visual_error"] = str(error)
        finally:
        ...
        try:
            ...
            speech_model = None if speech_api_key else get_speech_model()
            ...
            outcome["voice_evaluation"] = evaluate_voice_delivery_path(
                audio_path,
                transcript
            )
        except Exception as error:
            outcome["speech_error"] = str(error)
            failed_audio_path = locals().get("audio_path", "")
            if failed_audio_path and os.path.exists(failed_audio_path):
                os.remove(failed_audio_path)
    finally:
...
try:
    transcription_api_key = st.secrets.get("GROQ_API_KEY", "")
except Exception:
...
with st.sidebar:
    ...
    selected_transcription_language = st.selectbox(
        "Spoken language",
        TRANSCRIPTION_LANGUAGES,
        index=0,
        help=(
            "Choose the spoken language when known for better transcription. "
            "Use automatic detection for mixed-language answers."
        )
    )
    transcription_language = (
        "auto"
        if selected_transcription_language == "Detect automatically"
        else selected_transcription_language.lower()
    )
    st.divider()
    if api_key:
    ...
    if transcription_api_key:
        st.success("High-accuracy multilingual transcription is ready")
    else:
        st.warning(
            "Add GROQ_API_KEY to Streamlit Secrets for high-accuracy "
            "multilingual transcription. Local Whisper Base will be used "
            "until then."
        )
...
with setup_tab:
        ...
        if not target_role or not st.session_state.cv_text or not st.session_state.job_description:
            ...
            try:
                with st.spinner("The CV Analyst and Interviewer are preparing your studio..."):
                    ...
                    st.session_state.role_job_alignment = interview_setup.get(
                        "role_job_alignment",
                        {}
                    )
    ...
    if "cv_analysis" in st.session_state:
        ...
        role_job_alignment = st.session_state.get(
            "role_job_alignment",
            {}
        )
        if role_job_alignment:
            alignment_status = role_job_alignment.get(
                "status",
                "partial_match"
            )
            alignment_score = role_job_alignment.get("score", 0)
            alignment_summary = role_job_alignment.get(
                "summary",
                "The selected field and job requirements were compared."
            )

            if alignment_status == "match":
                st.success(
                    f"Selected major/role and job match: {alignment_score}%. "
                    + alignment_summary
                )
            elif alignment_status == "mismatch":
                st.error(
                    f"Possible major/role mismatch: {alignment_score}%. "
                    + alignment_summary
                )
            else:
                st.warning(
                    f"Partial major/role match: {alignment_score}%. "
                    + alignment_summary
                )

            alignment_differences = role_job_alignment.get(
                "differences",
                []
            )
            if alignment_differences:
                with st.expander("Why this notice appeared"):
                    for difference in alignment_differences:
                        st.write("• " + str(difference))

            st.caption(
                "This is a practice comparison of the selected field and the "
                "written job requirements. It is not an eligibility or hiring "
                "decision."
            )
...
with delivery_tab:
    ...
    media_file = st.file_uploader(
        "Upload an interview image or video",
        type=[
            "jpg", "jpeg", "png", "mp4", "mov", "avi", "webm", "mkv",
            "m4v"
        ],
        key="delivery_media"
    )
    if media_file is not None:
        ...
        media_extension = os.path.splitext(media_file.name)[1].lower()
        is_video_file = (
            media_file.type.startswith("video")
            or media_extension in {
        ...
        if is_video_file:
            if st.button(
                "Analyse complete uploaded interview",
                type="primary",
                use_container_width=True,
                key="analyse_complete_uploaded_interview"
            ):
                with st.spinner(
                    "Analysing the saved video first, then its speech and "
                    "voice delivery..."
                ):
                    complete_outcome = analyse_complete_uploaded_video(
                        media_file,
                        transcription_language,
                        transcription_api_key
                    )

                complete_fingerprint = media_fingerprint

                visual_result = complete_outcome.get("visual_result")
                if visual_result:
                    st.session_state.delivery_result = visual_result
                    st.session_state.delivery_media_fingerprint = (
                        complete_fingerprint
                    )
                    st.success("Visual video analysis completed.")
                else:
                    st.error(
                        "Visual-analysis stage failed: "
                        + complete_outcome.get(
                            "visual_error",
                            "Unknown visual-analysis error."
                        )
                    )

                gesture_warning = complete_outcome.get(
                    "gesture_warning",
                    ""
                )
                if gesture_warning:
                    st.warning(gesture_warning)

                transcript = complete_outcome.get("transcript", "")
                if transcript:
                    previous_audio_path = st.session_state.get(
                        "uploaded_video_audio_path",
                        ""
                    )
                    new_audio_path = complete_outcome.get("audio_path", "")
                    if (
                        previous_audio_path
                        and previous_audio_path != new_audio_path
                        and os.path.exists(previous_audio_path)
                    ):
                        os.remove(previous_audio_path)

                    st.session_state.uploaded_video_audio_path = (
                        new_audio_path
                    )
                    st.session_state.uploaded_video_audio_fingerprint = (
                        complete_fingerprint
                    )
                    st.session_state.uploaded_video_transcript = transcript
                    st.session_state.uploaded_video_corrected_transcript = (
                        transcript
                    )
                    st.session_state.uploaded_video_voice_evaluation = (
                        complete_outcome.get("voice_evaluation", {})
                    )
                    st.success(
                        "Speech transcription and voice-delivery analysis "
                        "completed. Check the transcript below."
                    )
                else:
                    st.error(
                        "Speech-analysis stage failed: "
                        + complete_outcome.get(
                            "speech_error",
        ...
        visual_only_label = (
            ...
            if is_video_file
            else "Run person + face-model observation"
        )
        if st.button(visual_only_label):
            try:
                if (
                        ...
                        try:
                            ...
                            if is_video_file:
                                try:
                                    ...
                                    gesture_recognizer = None
                                    st.warning(
                                        "MediaPipe is not installed, so hand "
                                        "gestures will be skipped. The other "
                                        "video observations will continue."
                                    )
        ...
        if is_video_file:
            st.markdown("### Uploaded-video speech and answer feedback")
            st.caption(
                "The app extracts the video's audio, creates an editable "
                "transcript and evaluates it against the currently selected "
                "Interview Room question."
            )

            uploaded_video_fingerprint = media_fingerprint
            if (
                st.session_state.get("uploaded_video_audio_fingerprint")
                ...
                st.session_state.uploaded_video_transcript = ""
                st.session_state.uploaded_video_corrected_transcript = ""
                st.session_state.pop(
                    "uploaded_video_voice_evaluation",
                    None
                )
                st.session_state.pop(
                    "uploaded_video_answer_evaluation",
                    None
                )

            if st.button(
                "Retry uploaded-video transcription only",
                use_container_width=True,
                key="transcribe_uploaded_video"
            ):
                try:
                    from speech_utils import (
                        extract_audio_from_video,
                    ...
                    from vision_utils import save_upload

                    if not transcription_api_key:
                        release_visual_models()
                    with st.spinner(
                        "Extracting the uploaded video's audio and running "
                        "Whisper transcription..."
                    ):
                        uploaded_video_path = save_upload(media_file)
                        try:
                            audio_path = extract_audio_from_video(
                                uploaded_video_path
                            )
                    ...
                    if (
                        old_audio_path
                        and old_audio_path != audio_path
                        and os.path.exists(old_audio_path)
                    ):
                        os.remove(old_audio_path)
                    st.session_state.uploaded_video_audio_path = audio_path
                    st.session_state.uploaded_video_transcript = transcript
                    st.session_state.uploaded_video_corrected_transcript = (
                        transcript
                    )
                    st.success(
                        "Transcript created. Correct any transcription "
                        "mistakes before requesting feedback."
                    )
                except Exception as error:
                    st.error(str(error))

            corrected_uploaded_transcript = st.text_area(
                "Uploaded-video answer / corrected transcript",
                key="uploaded_video_corrected_transcript",
                height=190,
                help=(
                    "Correct names, technical terms and transcription "
                    "mistakes before evaluating the answer."
                )
            )

            uploaded_voice_column, uploaded_answer_column = st.columns(2)
            with uploaded_voice_column:
                if st.button(
                    "Evaluate uploaded-video voice delivery",
                    use_container_width=True,
                    key="evaluate_uploaded_video_voice"
                ):
                    try:
                        from speech_utils import evaluate_voice_delivery_path
                        ...
                        if not audio_path or not os.path.exists(audio_path):
                            st.warning(
                                "Create the uploaded-video transcript first."
                            )
                        elif not corrected_uploaded_transcript.strip():
                            st.warning(
                                "Check the transcript before evaluating voice "
                                "delivery."
                            )
                        else:
                            st.session_state.uploaded_video_voice_evaluation = (
                                evaluate_voice_delivery_path(
                                    audio_path,
                                    corrected_uploaded_transcript
                                )
                            )
                    except Exception as error:
                        st.error(str(error))

            with uploaded_answer_column:
                if st.button(
                    "Ask the Answer Coach about this uploaded video",
                    type="primary",
                    use_container_width=True,
                    key="evaluate_uploaded_video_answer"
                ):
                    questions = st.session_state.get("questions", [])
                    if not questions:
                        st.warning(
                            "Complete Role Lab first so the uploaded answer "
                            "can be evaluated against an interview question."
                        )
                    elif not corrected_uploaded_transcript.strip():
                        st.warning(
                            "Create and check the uploaded-video transcript "
                            "first."
                        )
                    else:
                        try:
                            ...
                            uploaded_video_question = questions[
                                question_index
                            ].get("question", "")
                            with st.spinner(
                                "The Answer Coach is reviewing the uploaded "
                                "video answer..."
                            ):
                                uploaded_video_evaluation = evaluate_answer(
                                    uploaded_video_question,
                                    corrected_uploaded_transcript,
                                    st.session_state.cv_text,
                                    st.session_state.job_description,
                                    api_key
                            ...
                            st.session_state.uploaded_video_answer_evaluation = (
                                uploaded_video_evaluation
                            )
                            voice_evaluation = st.session_state.get(
                                "uploaded_video_voice_evaluation",
                                {}
                            ...
                            uploaded_record = {
                                "question": uploaded_video_question,
                                "answer": corrected_uploaded_transcript,
                                "evaluation": uploaded_video_evaluation,
                                "voice_evaluation": voice_evaluation
                            }
                            st.session_state.answer_results = [
                                item
                                for item in st.session_state.answer_results
                                if item["question"] != uploaded_video_question
                            ] + [uploaded_record]
                        except Exception as error:
                            st.error(str(error))

            uploaded_voice_result = st.session_state.get(
                "uploaded_video_voice_evaluation",
                {}
            )
            if uploaded_voice_result:
                st.markdown("#### Uploaded-video voice delivery")
                uploaded_voice_metrics = st.columns(3)
                uploaded_voice_metrics[0].metric(
                    "Duration",
                    f'{uploaded_voice_result.get("duration_seconds", 0)} sec'
                )
                uploaded_voice_metrics[1].metric(
                    "Speaking pace",
                    f'{uploaded_voice_result.get("words_per_minute", 0)} words/min'
                )
                uploaded_voice_metrics[2].metric(
                    "Listed filler words",
                    uploaded_voice_result.get("filler_count", 0)
                )
                for feedback_item in uploaded_voice_result.get("feedback", []):
                    st.write("→ " + feedback_item)

            uploaded_answer_result = st.session_state.get(
                "uploaded_video_answer_evaluation",
                {}
            )
            if uploaded_answer_result:
                st.markdown("#### Uploaded-video Answer Coach")
                uploaded_answer_metrics = st.columns(5)
                for column, label, key in zip(
                    uploaded_answer_metrics,
                    [
                        "Relevance",
                        "Specificity",
                        "Structure",
                    ...
                    column.metric(
                        label,
                        f'{uploaded_answer_result.get(key, 0)}%'
                    )
                for value in uploaded_answer_result.get("strengths", []):
                    st.write("✓ " + value)
                for value in uploaded_answer_result.get("improvements", []):
                    st.write("→ " + value)
    ...
    if "delivery_result" in st.session_state:
        ...
        if gestures:
            ...
            result.get("source") == "live_camera"
            or "analysed_frames" in result
        ):
