...
st.set_page_config(page_title="Interview Studio 2026", page_icon="✦", layout="wide")
...
html,body,[class*="css"]{font-family:'Manrope',sans-serif}.stApp{background:#07111f;color:#eaf2ff}
.block-container{max-width:1320px;padding-top:1.35rem;padding-bottom:4rem}
[data-testid="stSidebar"]{background:#0b1729;border-right:1px solid #1d3552}
.hero{position:relative;overflow:hidden;padding:42px;border-radius:28px;background:radial-gradient(circle at 85% 10%,#35e6c950,transparent 28%),linear-gradient(125deg,#102441,#173b61 60%,#0f5f69);border:1px solid #3ee8cd55;box-shadow:0 24px 70px #0008;margin-bottom:24px}
.hero h1{font-size:3.15rem;line-height:1.05;color:white;margin:8px 0}.hero p{color:#c8d9ee;font-size:1.08rem;max-width:790px}.eyebrow{color:#58f1d7;font-weight:800;letter-spacing:.14em;font-size:.78rem}
.glass{background:linear-gradient(145deg,#12243bd9,#0c192ad9);border:1px solid #294867;border-radius:20px;padding:20px;box-shadow:0 14px 40px #0005;min-height:142px}.glass b{color:#fff}.glass p{color:#9fb5cc;font-size:.9rem}
.question{padding:26px;background:linear-gradient(120deg,#123252,#10243b);border:1px solid #2e6d87;border-left:6px solid #55edcf;border-radius:18px;color:#f7fbff;font-size:1.25rem;font-weight:700}
.notice{padding:15px 18px;border-radius:14px;background:#182a3b;border:1px solid #36506a;color:#cbd9e8}
.agent{display:inline-block;padding:7px 12px;border-radius:30px;background:#173752;color:#65efd6;border:1px solid #2c6070;font-size:.8rem;font-weight:700;margin:3px}
h1,h2,h3{color:#f4f8ff!important}.stButton>button{background:linear-gradient(90deg,#20bfa7,#2779d8);color:white;border:0;border-radius:12px;font-weight:800;min-height:45px;box-shadow:0 8px 24px #1f9f9d45}
div[data-testid="stMetric"]{background:#101f32;border:1px solid #29435e;padding:16px;border-radius:16px}
[data-testid="stTabs"] button{font-weight:800}.stProgress>div>div{background:linear-gradient(90deg,#25c8ad,#3288ec)}
</style>
""", unsafe_allow_html=True)

st.markdown("""
<div class="hero"><div class="eyebrow">AI INTERVIEW STUDIO • 2026</div>
<h1>Practise with a team<br>built around your story.</h1>
...
with st.sidebar:
    ...
    api_key = st.text_input("OpenRouter API key", type="password", help="You can alternatively add OPENAI_API_KEY to Streamlit Secrets.")
    try:
        saved_api_key = st.secrets.get("OPENAI_API_KEY", "")
    except Exception:
        saved_api_key = ""
    api_key = api_key or saved_api_key
    target_role = st.text_input("Target role", placeholder="Example: HR Operations Executive")
    st.divider()
    st.caption("Four focused LLM roles use the tutor's OpenRouter API pattern. They are sequential prompts, not an external agent framework.")
    st.caption("Practice support only. Never use this app as an automated hiring decision system.")
...
with delivery_tab:
    ...
    if "delivery_result" in st.session_state:
        ...
        st.json(result)
        st.caption("Expression labels are predictions from the pretrained image model, not verified inner feelings.")
    ...
    if not answer_results:
        ...
        if "session_summary" in st.session_state:
            ...
            st.download_button(
                ...
                file_name="interview_practice_report.json",
                mime="application/json",
                use_container_width=True
            )

st.markdown("---")
st.caption("AI Interview Studio supports practice and reflection. It does not make recruitment decisions or infer protected traits.")
