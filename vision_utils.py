...
IMAGE_SIZE = (128, 128)
POSE_IMAGE_SIZE = 256
VIDEO_SAMPLES_PER_SECOND = 0.5
VIDEO_MAX_SAMPLES = 12
VIDEO_MIN_SAMPLES = 4
VIDEO_ANALYSIS_WIDTH = 640
GESTURE_MAX_SAMPLES = 8
...
def _resize_video_frame(frame, max_width=VIDEO_ANALYSIS_WIDTH):
    """Reduce large video frames before running CPU-based models."""
    if frame is None:
        return frame
    ...
    try:
        ...
        if total_frames > 0:
            ...
            for position in sample_positions:
                ...
                if success:
                    frames.append(_resize_video_frame(frame))
            return frames
...
def _analyse_gesture_frames(gesture_recognizer, frames):
    if gesture_recognizer is None or not frames:
        return [], ""

    try:
        import mediapipe as mp

        number_to_check = min(GESTURE_MAX_SAMPLES, len(frames))
        ...
        for index in selected_indices:
            ...
            label = "Unrecognized hand gesture"
            if result.gestures and result.gestures[0]:
                category_name = result.gestures[0][0].category_name or ""
                if category_name and category_name != "None":
                    label = category_name.replace("_", " ")
            gesture_counts[label] = gesture_counts.get(label, 0) + 1

        total_gestures = sum(gesture_counts.values())
        if not total_gestures:
            return [], ""

        gestures = [
            {
                "label": label,
                "score": count / total_gestures,
                "observations": count
            }
...
def analyse_video(
    ...
    gestures, gesture_error = _analyse_gesture_frames(
        gesture_recognizer,
        frames
    )
