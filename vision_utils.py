...
class LiveExpressionAnalyser:
    """Sample facial-expression outputs while a WebRTC camera stays live."""

    def __init__(self, expression_model_data, sample_interval=1.0):
    ...
    def _analyse_sample(self, image):
        ...
        try:
            prediction_batch = predict_face_batch(
                prepared_faces,
                self.expression_model_data
            )
            predictions = prediction_batch[0] if prediction_batch else []
        ...
        with self.lock:
            ...
            if predictions:
                ...
                self.latest_label = top_label
                self.label_counts[top_label] = (
                    self.label_counts.get(top_label, 0) + 1
                )
    ...
    def process_frame(self, frame):
        ...
        if latest_label:
            display_text = "Visible expression: " + latest_label.title()
            cv2.putText(
                image,
                display_text,
                (18, 34),
    ...
    def get_results(self):
        ...
        return {
            ...
            "expression_error": analysis_error,
            "confidence_outputs": [],
            "confidence_error": "",
